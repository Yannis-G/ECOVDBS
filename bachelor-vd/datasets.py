...
def fvecs_read(filename, bounds=None) -> np.ndarray[np.ndarray[np.float32]]:
    """
...
def ivecs_read(filename, bounds=None) -> np.ndarray[np.ndarray[np.int32]]:
    """
    ...
    with open(filename, 'rb') as fid:
        ...
        # Read n vectors
        v = np.fromfile(fid, dtype=np.int32, count=(d + 1) * n)
        v = np.reshape(v, (d + 1, n), order='F')
